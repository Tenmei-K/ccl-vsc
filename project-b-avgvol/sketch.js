...
function draw() {
  ...
  if (railStarLoc <= - 25) {
    ...
    text("👌(pinch index finger & thumb) = ⭐", 15, 30);
    text("🔊++ = ?", 16.5, 63);

    // 只执行一次
    if (interactionStart == false) {
      // 为vol avg记录interactionStart时的frameCount
      volAvgFrameCount = frameCount;
      // 重新push一整圈railStars
      railStars = [];
    ...
    // 计算avg
    volAvg = 0.9 * volAvg + 0.1 * vol; // 变化更显著
    volAvg2 = 0.999 * volAvg2 + 0.001 * vol;

    // 过大声音
    if (volAvg - volAvg2 > VOL_THRESHOLD) {
      for (let i = 0; i < bgRailStars.length; i++) {
        bgRailStars[i].dx *= map(volAvg - volAvg2 + VOL_THRESHOLD, 0, 0.1, 1, 3)
      }
      for (let i = 0; i < railStars.length; i++) {
        railStars[i].dx *= map(volAvg - volAvg2 + VOL_THRESHOLD, 0, 0.1, 1, 3)
      }
      for (let i = 0; i < stars.length; i++) {
        stars[i].dx *= map(volAvg - volAvg2 + VOL_THRESHOLD, 0, 0.1, 1, 3);
      }
    } else {
      for (let i = 0; i < bgRailStars.length; i++) {
